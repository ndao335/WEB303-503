...
function App() {
  ...
  const [getState, setGetState] = useState('Irvine, USA'); //getState to store the location name from input field
  const [state, setState] = useState('Irvine, USA'); //state to store a copy of the getState - this will be helpful in updating state on button click
  
  //API KEY and URL
  const apiKey = process.env.REACT_APP_API_KEY; //apiKey to store the API key created earlier in the .env file.
  const apiURL = `https://api.openweathermap.org/data/2.5/weather?q=${state}&appid=${apiKey}`; //apiURL to store the URL that calls the location from getState and also apiKey.
